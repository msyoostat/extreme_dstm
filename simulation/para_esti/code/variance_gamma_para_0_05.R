...
load("variance_gamma_3_data.RData")
...
stable_function<-function(x,
  ...
  current_log_likelihood<-current_log_likelihood_all[x]+
    ...
    dnorm(current_theta[x],mean=0,sd=5,log=TRUE)+ # prior for theta
  ...
  candidate_log_likelihood<-candidate_log_likelihood_all[x]+
    ...
    dnorm(proposal_theta[x],mean=0,sd=5,log=TRUE)+ # prior for theta
...
initial_value<-c(c(solve(t(H_mat %*% Phi_mat)%*%(H_mat %*% Phi_mat)) %*% t(H_mat %*%Phi_mat) %*% data.mat))



# initial and hyperparameter-----------------------------------------------------------------
hyper_alpha_nu1<-2
hyper_beta_nu1<-4.5*(hyper_alpha_nu1-1)

hyper_alpha_nu2<-2
hyper_beta_nu2<-0.35*(hyper_alpha_nu2-1)
hyper_alpha_nu2<-0.1
hyper_beta_nu2<- 0.1
...
proposal_sd_nu<-0.35
proposal_sd_theta<-0.35
proposal_sd_sigma<-0.35
proposal_delta_sd<-0.73
...
shape<-hyper_alpha_d + ( (time.length) *N)/2
...
for(j in 2:iterations){
  ...
  if(j %% 30==0){
    print(paste("theta=",theta_mat[1,j]))
    print(paste("sigma=",sigma_mat[1,j]))
...
#### save .RData file
##save.image("variance_gamma_para_0_05.RData")
