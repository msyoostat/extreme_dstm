...
## now location wise mean
#data.mat<-data.mat[-mask_locations,]
data.mat<-notLog( notExp( (data.mat) ) -1 )

predict.time<-c(50,100)

data.mat[,predict.time]<-NA

time_point_data<-(1:time.length)[-predict.time]
               ...
               if(t==1){
                ...
                arma::mat D= ( 
                  (1/epsilon_nuisance)*arma::eye( J,J ) +
                   (Phi_mat.t() * Phi_mat )/ sigma_square_d 
                  );
                arma::vec b= (
                  (M_mat * alpha_i.col(t-2))/epsilon_nuisance +
                  delta_mat.col(t-2)/epsilon_nuisance+ 
                  (Phi_mat.t() * ( y_mat.col(t-2) ) *1/sigma_square_d)
                );
                arma::mat D_inverse =arma::inv_sympd	(D);
                alpha_i.col(t-1)=arma::mvnrnd( D_inverse * b,D_inverse);

              } else if(  any(t==(predict_time) ) ){
                arma::mat D= (
                  (1/epsilon_nuisance)*arma::eye( J,J )+
...
stable_function<-function(x,
  ...
  current_log_likelihood<-current_log_likelihood_all[x]+
    ...
    log(dtruncnorm(current_lambda_1[x],a=0,b=2,mean=1,sd=0.12))
  candidate_log_likelihood<-candidate_log_likelihood_all[x]+
    ...
    log(dtruncnorm(proposal_lambda_1[x],a=0,b=2,mean=1,sd=0.12))
...
## lines 368~369: hyper parameters for the variance parameter in Gaussian
hyper_alpha_nu2<-2.5
...
###
post.alpha.mat[[1]][,predict.time[1]+1]<-apply(post.alpha.mat[[1]][,c(predict.time[1],predict.time[1]+2)],1,mean )
post.alpha.mat[[1]][,predict.time[2]+1]<-apply(post.alpha.mat[[1]][,c(predict.time[2],predict.time[2]+2)],1,mean )
...
proposal_sd_nu<-0.15
proposal_sd_lambda<-0.15
proposal_sd_kappa<-0.15
proposal_beta_sd<-1
proposal_delta_sd<-3.9
...
shape<-hyper_alpha_d + ( (time.length-2) *n_data)/2

Diagonal_J<-Diagonal(J)

#### storage for the predict.time
prediction_mat_mask_1<-matrix(nrow=N,ncol=iterations-burnin+1)
prediction_mat_mask_2<-matrix(nrow=N,ncol=iterations-burnin+1)
...
for(j in 2:iterations){
  parameter.mat.sigma.square.d[j]<-rinvgamma(1, shape,rate=(sum( ( (data.mat-H_mat%*%Phi_mat%*%post.alpha.mat.previous[,-1])[,-c(predict.time)] )^2 ) +2*hyper_beta_d )/2 )
  ...
  if(j %% 30==0){
    print(paste("lambda1=",lambda1_mat[43,j]))
    print(paste("nu1=",nu1_mat[43,j]))
    print(paste("nu2=",nu2_mat[43,j]))
    print(paste("hit ratio for nu1=", mean(accept.nu_lambda_ratio[43,j:(j-29)]) ) )
  ...
  if(j>=burnin){
    ...
    ## save predictions
    prediction_mat_mask_1[,j-burnin+1]<-(temptemp+locationwise_mean)[,predict.time[1]]
    prediction_mat_mask_2[,j-burnin+1]<-(temptemp+locationwise_mean)[,predict.time[2]]
...
#length( apply(accept.delta.ratio,1,mean) )
apply(accept.delta.ratio,1,mean) 
#accept.delta.ratio<-accept.delta.ratio[,-c(1:burnin)]
...
#### save .RData file
save.image("pm_2_5_missing_stable.RData")
