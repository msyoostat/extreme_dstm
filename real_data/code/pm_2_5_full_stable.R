...
## prediction
# prediction_mat<-matrix(NA,nrow=time.length,ncol=iterations)
...
for(j in 2:iterations){
  ...
  #parameter.mat.sigma.square.d[j]<-rinvgamma(1, shape,rate=(sum( ( (data.mat-Phi_mat%*%post.alpha.mat[[j-1]][,-1])[,-c(predict.time)] )^2 ) +2*hyper_beta_d )/2 )
  # parameter.mat.sigma.square.d[j]<-sigma_square_d
...
length(parameter.mat.beta.list)
for(j in 1:J){
...
apply(accept.nu_lambda_ratio,1,mean)[100]


apply(accept.beta.ratio,1,mean)[91]

# save --------------------------------------------------------------------

#### save .RData file
save.image("pm_2_5_full_stable.RData")
